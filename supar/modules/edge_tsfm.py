...
class EdgeAttention(nn.Module):
	def __init__(self,d_model, num_heads, dropout):
		...
		# self.w_vl = nn.Linear(d_model, d_model)
		# self.w_vr = nn.Linear(d_model, d_model)
		# self.w_o = nn.Linear(d_model, d_model)
		# self.dropout11 = nn.Dropout(p=dropout)
		# self.dropout12 = nn.Dropout(p=dropout)
		# self.dropout13 = nn.Dropout(p=dropout)
		# self.dropout14 = nn.Dropout(p=dropout)

		self.linears = _get_clones(nn.Linear(d_model, d_model), 6)
		self.attn = None
		self.dropout = nn.Dropout(p=dropout)

		self.lesion_scores = False
		self.lesion_values = False

	def forward(self, query, key, value, mask=None):
		num_batches = query.size(0)
		num_nodes = query.size(1)
		
		# q = self.w_q(query).view(num_batches, num_nodes, num_nodes, self.num_heads, self.d_k)
		# k = self.w_k(query).view_as(q)
		# vl = self.w_vl(query).view_as(q)
		# vr = self.w_vr(query).view_as(q)
		...
		# scores1 = torch.einsum("bxahd,bayhd->bxayh", q, k) / math.sqrt(self.d_k)
		# scores2 = torch.einsum("bxahd,byahd->bxayh", q, k) / math.sqrt(self.d_k)
		# scores3 = torch.einsum("baxhd,bayhd->bxayh", q, k) / math.sqrt(self.d_k)
		# scores4 = torch.einsum("baxhd,byahd->bxayh", q, k) / math.sqrt(self.d_k)
		# if mask is not None:
		# 	scores1 = scores1.masked_fill(mask.unsqueeze(4), -1e3)
		# 	scores2 = scores2.masked_fill(mask.unsqueeze(4), -1e3)
		# 	scores3 = scores3.masked_fill(mask.unsqueeze(4), -1e3)
		# 	scores4 = scores4.masked_fill(mask.unsqueeze(4), -1e3)
		# score = torch.cat((scores1, scores2, scores3, scores4), dim=2)
		# att = nn.functional.softmax(score, dim=2)
		# att = self.dropout(att)
		# att = att.view(num_batches, num_nodes, 4, num_nodes, num_nodes, self.num_heads)

		# # att1 = nn.functional.softmax(scores1, dim=2)
		# # att2 = nn.functional.softmax(scores2, dim=2)
		# # att3 = nn.functional.softmax(scores3, dim=2)
		# # att4 = nn.functional.softmax(scores4, dim=2)
		# # att1 = self.dropout11(att1)
		# # att2 = self.dropout12(att2)
		# # att3 = self.dropout13(att3)
		# # att4 = self.dropout14(att4)

		# # x = torch.einsum("bxayh,bxahd->bxyhd", att[:, :, 0, :, :, :], vl).contiguous().view(num_batches, num_nodes, num_nodes, self.d_model)
		# # x = x + torch.einsum("bxayh,byahd->bxyhd", att[:, :, 1, :, :, :], vl).contiguous().view_as(x)
		# # x = x + torch.einsum("bxayh,baxhd->bxyhd", att[:, :, 2, :, :, :], vl).contiguous().view_as(x)
		# # x = x + torch.einsum("bxayh,bayhd->bxyhd", att[:, :, 3, :, :, :], vl).contiguous().view_as(x)

		# # x = torch.einsum("bxayh,bxahd->bxyhd", att1, vl).contiguous().view(num_batches, num_nodes, num_nodes, self.d_model)
		# # x = x + torch.einsum("bxayh,byahd->bxyhd", att2, vl).contiguous().view_as(x)
		# # x = x + torch.einsum("bxayh,baxhd->bxyhd", att3, vl).contiguous().view_as(x)
		# # x = x + torch.einsum("bxayh,bayhd->bxyhd", att4, vl).contiguous().view_as(x)

		# v1 = torch.einsum("bxahd,bayhd->bxayhd", vl, vr)
		# v2 = torch.einsum("bxahd,byahd->bxayhd", vl, vr)
		# v3 = torch.einsum("baxhd,bayhd->bxayhd", vl, vr)
		# v4 = torch.einsum("baxhd,byahd->bxayhd", vl, vr)
		# x = torch.einsum("bxayh,bxayhd->bxyhd", att[:, :, 0, :, :, :], v1).contiguous().view(num_batches, num_nodes, num_nodes, self.d_model)
		# x = x + torch.einsum("bxayh,bxayhd->bxyhd", att[:, :, 1, :, :, :], v2).contiguous().view_as(x)
		# x = x + torch.einsum("bxayh,bxayhd->bxyhd", att[:, :, 2, :, :, :], v3).contiguous().view_as(x)
		# x = x + torch.einsum("bxayh,bxayhd->bxyhd", att[:, :, 3, :, :, :], v4).contiguous().view_as(x)

		# # x = torch.einsum("bxayh,bxayhd->bxyhd", att1, v1).contiguous().view(num_batches, num_nodes, num_nodes, self.d_model)
		# # x = x + torch.einsum("bxayh,bxayhd->bxyhd", att2, v2).contiguous().view_as(x)
		# # x = x + torch.einsum("bxayh,bxayhd->bxyhd", att3, v3).contiguous().view_as(x)
		# # x = x + torch.einsum("bxayh,bxayhd->bxyhd", att4, v4).contiguous().view_as(x)
		# return self.w_o(x)
		...
		if mask is not None:
			scores = scores.masked_fill(mask.unsqueeze(4).lt(0.5), -1e3)
...
class EdgeTransformerLayer(nn.Module):
	...
	def __init__(self, input_size, activation="relu"):
		...
		self.edge_attention_flat = False

		if self.edge_attention_flat:
			self.edge_attention = EdgeAttentionFlat(d_model,self.num_heads, dropout)
		else:
			self.edge_attention = EdgeAttention(d_model,self.num_heads, dropout)
...
class EdgeTransformerEncoder(nn.Module):
	...
	def __init__(self,num_layers,input_size,activation="relu"):
		super().__init__()
		...
		# self.wq = nn.Parameter(torch.Tensor(self.model_config['dim'], self.model_config['dim'], self.model_config['rank']))
		# self.wk = nn.Parameter(torch.Tensor(self.model_config['dim'], self.model_config['dim'], self.model_config['rank']))
		
		# self.wq = nn.Parameter(torch.Tensor(input_size, input_size))
		# self.wk = nn.Parameter(torch.Tensor(input_size, input_size))
		# nn.init.xavier_normal_(self.wq)
		# nn.init.xavier_normal_(self.wk)

		encoder_layer = EdgeTransformerLayer(input_size)
		...
		# self.spatial_relations_builder = SpatialRelationsBuilder(self.model_config['rank'])
	...
	def forward(self, edge_inputs, mask=None):
		...
		# left_inputs = torch.einsum("bxd,dhr->bxhr", edge_inputs, self.wq)
		# right_inputs = torch.einsum("byd,dhr->byhr", edge_inputs, self.wk)
		# batch = torch.einsum("bxhr,byhr->bxyh", left_inputs, right_inputs)

		# left_inputs = torch.einsum("bxd,dh->bxh", edge_inputs, self.wq)
		# right_inputs = torch.einsum("byd,dh->byh", edge_inputs, self.wk)
		# batch = torch.einsum("bxh,byh->bxyh", left_inputs, right_inputs)

		batch += self.spatial_relations_builder(num_nodes)

		# batch = batch + torch.einsum("bxhr,xyr->bxyh", left_inputs, self.spatial_relations_builder(num_nodes))
		
		# batch = batch + torch.einsum("bxh,xyh->bxyh", left_inputs, self.spatial_relations_builder(num_nodes))
		...
		for mod in self.layers:
			...
			batches = batches + (batch.permute(0, 3, 1, 2),)
...
class Parser(pl.LightningModule):
	...
	def _create_model(self,args):
		...
		if self.args['model_type']=='roberta':
			config = AutoConfig.from_pretrained(f"{self.args['bert']}.json", vocab_size=self.args['token_vocab_size'], output_hidden_states=True, output_attentions=True)
			# self.encoder = RobertaModel(config, custom=True, rank=self.args['dim'], dim=self.args['rank'], cpd=True, softmax_head=False, edge_attn=False)
			# self.encoder = RobertaModel.from_pretrained("roberta-base", local_files_only=True)
			# self.decoder_q = nn.Linear(self.args['dim'], self.args['dim'])
			# self.decoder_k = nn.Linear(self.args['dim'], self.args['dim'])
		else:
	...
	def _calculate_loss(self, batch):
		...
		if self.args['model_type']=='roberta':
			outputs = self.encoder(tokens,1-mask.long()).attentions[-1].permute(0, 2, 3, 1)
			...
			# node_outputs = self.encoder(tokens,1-mask.long()).last_hidden_state
			# q = self.decoder_q(node_outputs)
			# k = self.decoder_k(node_outputs)
			# deprel_scores = torch.bmm(q,k.transpose(1,2))

		elif self.args['model_type']=='edge_transformer':
